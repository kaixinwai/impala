...
const impalaSecondaryEntryPoints = getSubdirectoryNames(join(buildConfig.packagesDir, 'lib'));
...
const rollupImpalaEntryPoints = impalaSecondaryEntryPoints.reduce((globals: any, entryPoint: string) => {
    globals[`impala/${entryPoint}`] = `impala.${dashCaseToCamelCase(entryPoint)}`;
    return globals;
}, {});
